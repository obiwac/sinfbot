...
export default {
  ...
  once: false,
  async execute(client: Client, reaction: MessageReaction, user: User): Promise<void> {
    let isPin = false;
    ...
    const messageAffect = await reaction.message.channel.messages.fetch(messageId).catch();
    if (!messageAffect) {
      return;
    }
        ...
        if (
          ...
          const reactEmbed = new EmbedBuilder()
            ...
            .setDescription(
              ...
              ${isPin ? 'pin' : 'unpin'} (${totalAgainst} vs ${totalFavour}). Message will not be 
              ${isPin ? 'pinned' : 'unpinned'}.`
            )
          ...
          const reactEmbed = new EmbedBuilder()
            .setTitle(isPin ? 'Pin vote' : 'Unpin vote')
            .setDescription(
              `Vote was successful! Message will now be ${isPin ? 'pinned' : 'unpinned'}!`
            )
            .setThumbnail(ICONS['salute'])
            .setColor(0x00ff00);
          await message.reply({ embeds: [reactEmbed] });
