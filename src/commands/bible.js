...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption(option =>
      option.setName('chapter').setDescription('Chapter').setRequired(true),
    )
    .addIntegerOption(option =>
      option.setName('verse').setDescription('Verse').setRequired(true),
    ),
  ...
  async execute(client, interaction) {
    ...
    req.on('error', error => {
      return interaction.reply({
        content: 'Error while trying to get the Bible',
        ephemeral: true,
      })
    })
