...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('id').setDescription('ID of the message you want to unpin').setRequired(true)
    ),
  async execute(client: Client, interaction: CommandInteraction): Promise<void> {
    ...
    if (!message) {
      await interaction.reply({
        content: "Message could not be found! Are you sure you're passing a valid message ID?",
        ephemeral: true,
      });
      return;
    }
