const { ActionRowBuilder, ButtonBuilder, EmbedBuilder, SlashCommandBuilder } = require('@discordjs/builders')
...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        const row = new ActionRowBuilder()
            .addComponents(approveButton, rejectButton, warnButton);

        if (messageAttachment !== null) confessionEmbed.setImage(messageAttachment.url);

        let vote = await adminChannel.send({ embeds: [confessionEmbed], components: [row] });

        const collector = vote.createMessageComponentCollector({ componentType: ComponentType.Button, time: VOTE_MINUTES * 60 * 1000});
