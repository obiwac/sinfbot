  ...
  async execute(client, interaction) {
    ...
    collector.on('collect', (reaction, user) => {
      ...
      if (member.roles.cache.some(role => role.name === ROLE_NAME)) {
        ...
        if (name === IN_FAVOUR_REACTION) {
          const rich = gen_rich(
            `User with the ${ROLE_NAME} role (${user.tag}) voted to pin the message! Message will now be pinned!`,
            'salute',
            0x00ff00,
          )
        ...
        const rich = gen_rich(
          `User with the ${ROLE_NAME} role (${user.tag}) voted to dismiss the message. Message will not be pinned.`,
          'uncool',
          0xff0000,
        )
        vote.reply({ embeds: [rich] })
    ...
    collector.on('end', _ => {
      ...
      if (in_favour < VOTE_THRESHOLD) {
        ...
        if (in_favour == 0) {
          rich = gen_rich(
            `No one voted in favour of pinning this message (threshold is ${VOTE_THRESHOLD}). Message will not be pinned.`,
            'silly',
            0xff0000,
          )
        } else if (in_favour == 1) {
          rich = gen_rich(
            `Only one person voted in favour of pinning this message (threshold is ${VOTE_THRESHOLD}). Message will not be pinned.`,
            'sadge',
            0xff0000,
          )
        } else {
          rich = gen_rich(
            `Only ${in_favour} people voted in favour of pinning this message (threshold is ${VOTE_THRESHOLD}). Message will not be pinned.`,
            'uncool',
            0xff0000,
          )
        }
      ...
      if (against >= in_favour) {
        const rich = gen_rich(
          `More or the same number of people voted against the pin as in favour of the pin (${against} vs ${in_favour}). Message will not be pinned.`,
          'waah',
          0xff0000,
        )
      ...
      const rich = gen_rich(
        'Vote was successful! Message will now be pinned!',
        'salute',
        0x00ff00,
      )
      vote.reply({ embeds: [rich] })
